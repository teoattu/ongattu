<script>
  import Button from "../components/Button.svelte";
import Card from "../components/Card.svelte";
import DropDownListItem from "../components/DropDownListItem.svelte";
  import "../css/global.css";
</script>

<svelte:head>
  <title>ONG ATTU HOMEPAGE</title>
</svelte:head>

<div class="wrapper">
  <section class="index__main">
    <h1>Bună, noi suntem attu!</h1>
    <p>
      Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam erat, sed diam voluptua. At vero eos et accusam et justo duo dolores et ea rebum. Stet clita kasd gubergren, no sea.
    </p>
    <div class="index__button__wrapper">
      <Button type="outlined">Cunoaște-ne mai bine</Button>
    </div>
  </section>
  <section class="index__7ani">
    <div class="index__7ani__wrapper">
      <h3>7 ani de fapte bune!</h3>
      <p>
        Lorem ipsum dolor sit amet, consectetur adipiscing elit. Phasellus ac
        turpis in velit luctus hendrerit
      </p>
      <ul class="index__7ani__list">
        <li class="index__7ani__listitem">
          <svg xmlns="http://www.w3.org/2000/svg" width="80.662" height="60.769" viewBox="0 0 80.662 60.769">
            <g id="Group_294" data-name="Group 294" transform="translate(-423.852 -3305.309)">
              <path id="Path_1213" data-name="Path 1213" d="M67.6,31.137H58.959c0-4.44-1.53-10.073-5.674-11.627L30.491,11H1V50.568H23.2V45.82L48.993,51.7,78.855,45.82V40.969C78.855,34.826,73.747,31.137,67.6,31.137ZM5.717,44.3V16.124H18.87V44.3ZM48.993,45.82,23.2,38.678V16.124h7.29l21.028,8.37c1.258.481,1.765,5.274,1.765,6.643,0,0-8.457.37-9.6,0l-8.636-3.189-1.89,5.1,8.807,2.923a18.441,18.441,0,0,0,5.846.962h23.74c3.321,1.373,2.33,4.137,2.922,5.358Z" transform="translate(422.852 3314.375)" fill="#100abf"/>
              <g id="paid_black_24dp" transform="translate(471.472 3305.309)">
                <g id="Group_287" data-name="Group 287" transform="translate(0 0)">
                  <rect id="Rectangle_356" data-name="Rectangle 356" width="33.041" height="33.041" fill="none"/>
                </g>
                <g id="Group_288" data-name="Group 288" transform="translate(2.392 1.837)">
                  <path id="Path_1214" data-name="Path 1214" d="M16.1,2a14.1,14.1,0,1,0,14.1,14.1A14.1,14.1,0,0,0,16.1,2Zm1.241,22.217v.508A1.242,1.242,0,0,1,16.1,25.966h0a1.242,1.242,0,0,1-1.241-1.241v-.592a5.652,5.652,0,0,1-3.792-2.96,1.215,1.215,0,0,1,.634-1.663l.1-.042a1.2,1.2,0,0,1,1.523.55,3.3,3.3,0,0,0,2.989,1.931c1.311,0,2.791-.677,2.791-2.27,0-1.353-.987-2.058-3.214-2.862-1.551-.55-4.723-1.452-4.723-4.666a4.339,4.339,0,0,1,3.694-4.173V7.47A1.242,1.242,0,0,1,16.1,6.229h0A1.242,1.242,0,0,1,17.338,7.47v.522a4.777,4.777,0,0,1,3.045,1.833,1.2,1.2,0,0,1-.508,1.833h0a1.232,1.232,0,0,1-1.438-.395,2.68,2.68,0,0,0-2.256-1.085c-.987,0-2.552.522-2.552,1.96,0,1.339,1.212,1.847,3.722,2.678,3.383,1.17,4.243,2.89,4.243,4.864C21.6,23.386,18.071,24.091,17.338,24.217Z" transform="translate(-2 -2)" fill="#100abf"/>
                </g>
              </g>
            </g>
          </svg>
          <div class="index__7ani__listitem__textarea">
            <h4>+250K RON investit</h4>
            <p>din donații și sponsorizării</p>
          </div>
        </li>
        <li class="index__7ani__listitem">
          <svg id="groups_black_24dp" xmlns="http://www.w3.org/2000/svg" width="82" height="82" viewBox="0 0 82 82">
            <rect id="Rectangle_353" data-name="Rectangle 353" width="82" height="82" fill="none"/>
            <g id="Group_281" data-name="Group 281" transform="translate(0 20.5)">
              <path id="Path_1210" data-name="Path 1210" d="M13.667,29.917a6.833,6.833,0,1,0-6.833-6.833A6.853,6.853,0,0,0,13.667,29.917Zm3.861,3.758a23.857,23.857,0,0,0-3.861-.342,23.748,23.748,0,0,0-9.5,1.982A6.87,6.87,0,0,0,0,41.636V47H15.375V41.5A15.37,15.37,0,0,1,17.528,33.675Zm50.806-3.758A6.833,6.833,0,1,0,61.5,23.083,6.853,6.853,0,0,0,68.333,29.917ZM82,41.636a6.87,6.87,0,0,0-4.168-6.321,23.227,23.227,0,0,0-13.359-1.64A15.37,15.37,0,0,1,66.625,41.5V47H82Zm-26.513-9.5A35.681,35.681,0,0,0,41,29.062a36.254,36.254,0,0,0-14.487,3.075A10.209,10.209,0,0,0,20.5,41.5V47h41V41.5A10.209,10.209,0,0,0,55.487,32.137ZM27.572,40.167c.308-.786.444-1.333,3.109-2.357a28.779,28.779,0,0,1,20.637,0c2.631,1.025,2.768,1.572,3.109,2.357ZM41,12.833a3.417,3.417,0,1,1-3.417,3.417A3.427,3.427,0,0,1,41,12.833M41,6A10.25,10.25,0,1,0,51.25,16.25,10.236,10.236,0,0,0,41,6Z" transform="translate(0 -6)" fill="#100abf"/>
            </g>
          </svg>
          <div class="index__7ani__listitem__textarea">
            <h4>+8000</h4>
            <p>de oamenii sprijiniți</p>
          </div>
        </li>

        <li class="index__7ani__listitem">
          <svg xmlns="http://www.w3.org/2000/svg" width="79" height="79" viewBox="0 0 79 79">
            <g id="interests_black_24dp" transform="translate(0 0.201)">
              <rect id="Rectangle_354" data-name="Rectangle 354" width="79" height="79" transform="translate(0 -0.201)" fill="none"/>
              <path id="Path_1211" data-name="Path 1211" d="M16.877,36.971A13.132,13.132,0,1,0,30.008,50.1,13.128,13.128,0,0,0,16.877,36.971Zm19.632,3.283v19.7a3.293,3.293,0,0,0,3.283,3.283h19.7a3.293,3.293,0,0,0,3.283-3.283v-19.7a3.293,3.293,0,0,0-3.283-3.283h-19.7A3.293,3.293,0,0,0,36.509,40.254ZM13.955,6.013,3.121,25.514a3.289,3.289,0,0,0,2.856,4.892H27.645A3.289,3.289,0,0,0,30.5,25.514L19.667,6.013A3.273,3.273,0,0,0,13.955,6.013ZM57.027,2.5a8.969,8.969,0,0,0-7.387,3.841,9.026,9.026,0,0,0-16.415,5.187c0,6.008,6.664,10.407,14.281,17.006a3.284,3.284,0,0,0,4.268,0c7.617-6.6,14.281-11,14.281-17.006A9.043,9.043,0,0,0,57.027,2.5Z" transform="translate(6.17 5.715)" fill="#100abf"/>
            </g>
          </svg>
          <div class="index__7ani__listitem__textarea">
            <h4>+60</h4>
            <p>proiecte desfășurate</p>
          </div>
        </li>
      </ul>
      
    </div>
  </section>
  <section class="index__activitati">
    <h2>Descoperă activitățile noastre!</h2>
    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor</p>

    <div class="index__activitati__cardarea">
      <Card src="/images/index/Educatie.jpeg">
        <h4 slot="header">Educație</h4>
        <span slot="content">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer faucibus est non purus posuere feugiat</span>
        <span slot="footer">Afla mai multe </span>
      </Card>
      <Card src="/images/index/Social.jpg">
        <h4 slot="header">Social</h4>
        <span slot="content">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer faucibus est non purus posuere feugiat</span>
        <span slot="footer">Afla mai multe </span>
      </Card>
    </div>
  </section>
  <section class="index__implicare">
    <h2>Cum te poți impica</h2>
    <div class="index__implicare__list">
      <DropDownListItem>
        <div slot="icon" style="padding-left: 2rem;"> 
          <svg xmlns="http://www.w3.org/2000/svg" width="40" height="30" viewBox="0 0 80.662 60.769">
            <g id="Group_294" data-name="Group 294" transform="translate(-423.852 -3305.309)">
              <path id="Path_1213" data-name="Path 1213" d="M67.6,31.137H58.959c0-4.44-1.53-10.073-5.674-11.627L30.491,11H1V50.568H23.2V45.82L48.993,51.7,78.855,45.82V40.969C78.855,34.826,73.747,31.137,67.6,31.137ZM5.717,44.3V16.124H18.87V44.3ZM48.993,45.82,23.2,38.678V16.124h7.29l21.028,8.37c1.258.481,1.765,5.274,1.765,6.643,0,0-8.457.37-9.6,0l-8.636-3.189-1.89,5.1,8.807,2.923a18.441,18.441,0,0,0,5.846.962h23.74c3.321,1.373,2.33,4.137,2.922,5.358Z" transform="translate(422.852 3314.375)" fill="#100abf"/>
              <g id="paid_black_24dp" transform="translate(471.472 3305.309)">
                <g id="Group_287" data-name="Group 287" transform="translate(0 0)">
                  <rect id="Rectangle_356" data-name="Rectangle 356" width="33.041" height="33.041" fill="none"/>
                </g>
                <g id="Group_288" data-name="Group 288" transform="translate(2.392 1.837)">
                  <path id="Path_1214" data-name="Path 1214" d="M16.1,2a14.1,14.1,0,1,0,14.1,14.1A14.1,14.1,0,0,0,16.1,2Zm1.241,22.217v.508A1.242,1.242,0,0,1,16.1,25.966h0a1.242,1.242,0,0,1-1.241-1.241v-.592a5.652,5.652,0,0,1-3.792-2.96,1.215,1.215,0,0,1,.634-1.663l.1-.042a1.2,1.2,0,0,1,1.523.55,3.3,3.3,0,0,0,2.989,1.931c1.311,0,2.791-.677,2.791-2.27,0-1.353-.987-2.058-3.214-2.862-1.551-.55-4.723-1.452-4.723-4.666a4.339,4.339,0,0,1,3.694-4.173V7.47A1.242,1.242,0,0,1,16.1,6.229h0A1.242,1.242,0,0,1,17.338,7.47v.522a4.777,4.777,0,0,1,3.045,1.833,1.2,1.2,0,0,1-.508,1.833h0a1.232,1.232,0,0,1-1.438-.395,2.68,2.68,0,0,0-2.256-1.085c-.987,0-2.552.522-2.552,1.96,0,1.339,1.212,1.847,3.722,2.678,3.383,1.17,4.243,2.89,4.243,4.864C21.6,23.386,18.071,24.091,17.338,24.217Z" transform="translate(-2 -2)" fill="#100abf"/>
                </g>
              </g>
            </g>
          </svg>
        </div>
        <h4 slot="title">Doneaza</h4>
        <div slot="content">
          <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer faucibus est non purus posuere feugiat. Sed nibh turpis, eleifend sit amet fermentum sed, cursus a ante. Morbi non eleifend massa. Maecenas nec facilisis lacus, in aliquam lacus.</p>
        </div>
      </DropDownListItem>
      <DropDownListItem>
        <div slot="icon" style="padding-left: 2rem;"> 
          <svg id="emoji_people_black_24dp" xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 59 59">
            <g id="Group_291" data-name="Group 291">
              <rect id="Rectangle_358" data-name="Rectangle 358" width="59" height="59" fill="none"/>
              <rect id="Rectangle_359" data-name="Rectangle 359" width="59" height="59" fill="none"/>
            </g>
            <g id="Group_293" data-name="Group 293" transform="translate(10.028 4.453)">
              <g id="Group_292" data-name="Group 292" transform="translate(0 0)">
                <ellipse id="Ellipse_327" data-name="Ellipse 327" cx="5.5" cy="5" rx="5.5" ry="5" transform="translate(13.972 -0.453)" fill="#100abf"/>
                <path id="Path_1216" data-name="Path 1216" d="M33.114,17.02a7.626,7.626,0,0,0-5.8-2.729H21.068a12.348,12.348,0,0,1-12.1-10.2A2.453,2.453,0,0,0,6.564,2h0A2.469,2.469,0,0,0,4.106,4.8,17.3,17.3,0,0,0,16.176,18.5V48.708a2.466,2.466,0,0,0,2.458,2.458h0a2.466,2.466,0,0,0,2.458-2.458V36.417H26.01V48.708a2.466,2.466,0,0,0,2.458,2.458h0a2.466,2.466,0,0,0,2.458-2.458V21.79l7.965,7.965a2.448,2.448,0,0,0,3.466,0h0a2.448,2.448,0,0,0,0-3.466Z" transform="translate(-4.079 -1.536)" fill="#100abf"/>
              </g>
            </g>
          </svg>
          
        </div>
        <h4 slot="title">Devino voluntar attu</h4>
        <div slot="content">
          <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer faucibus est non purus posuere feugiat. Sed nibh turpis, eleifend sit amet fermentum sed, cursus a ante. Morbi non eleifend massa. Maecenas nec facilisis lacus, in aliquam lacus.</p>
        </div>
      </DropDownListItem>
      <DropDownListItem>
        <div slot="icon" style="padding-left: 2rem;"> 
          <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 56 56">
            <g id="alt_route_black_24dp" transform="translate(0 0.287)">
              <rect id="Rectangle_357" data-name="Rectangle 357" width="56" height="56" transform="translate(0 -0.287)" fill="none"/>
              <path id="Path_1215" data-name="Path 1215" d="M17.807,23l-3.3,3.3a16.9,16.9,0,0,1-4.155-6.825l4.5-1.137A12.411,12.411,0,0,0,17.807,23Zm.859-13.951-6.5-6.5a1.149,1.149,0,0,0-1.648,0l-6.477,6.5a1.161,1.161,0,0,0,.836,1.973h4.2a34.774,34.774,0,0,0,.441,5.037l4.5-1.137a30.676,30.676,0,0,1-.3-3.9h4.132A1.159,1.159,0,0,0,18.666,9.048Zm23.214,0L35.4,2.571a1.149,1.149,0,0,0-1.648,0L27.278,9.048a1.149,1.149,0,0,0,.812,1.973h4.132c-.232,8.543-2.971,11.027-5.9,13.65a20.13,20.13,0,0,0-3.366,3.6,16.557,16.557,0,0,0-2.623-2.879l-3.273,3.273c2.159,1.973,3.575,3.575,3.575,7.893h0v9.285a2.328,2.328,0,0,0,2.321,2.321h0a2.328,2.328,0,0,0,2.321-2.321V36.556h0c0-4.689,1.648-6.175,4.155-8.427,3.2-2.879,7.15-6.453,7.428-17.109h4.178A1.161,1.161,0,0,0,41.879,9.048Z" transform="translate(4.896 2.907)" fill="#100abf"/>
            </g>
          </svg>          
          
        </div>
        <h4 slot="title">Redirecționează 20% din impozitul pe profit</h4>
        <div slot="content">
          <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer faucibus est non purus posuere feugiat. Sed nibh turpis, eleifend sit amet fermentum sed, cursus a ante. Morbi non eleifend massa. Maecenas nec facilisis lacus, in aliquam lacus.</p>
        </div>
      </DropDownListItem>
      <DropDownListItem>
        <div slot="icon" style="padding-left: 2rem;"> 
          <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 46.292 41.802">
            <path id="_12389786231582000618" data-name="12389786231582000618" d="M106.6,86.2a13.214,13.214,0,0,0-14.222.748,1.857,1.857,0,0,1-2.523-.012A13.732,13.732,0,0,0,72.072,88.87a13.271,13.271,0,0,0-.664,17.451,4.889,4.889,0,0,1,1.183,2.423,3.512,3.512,0,0,0,3.1,3.053c.93.166,1.207.475,1.364,1.33a3.62,3.62,0,0,0,3.091,3.089c1.026.178,1.268.641,1.449,1.509a3.613,3.613,0,0,0,3.042,2.922c.99.178,1.244.582,1.461,1.461a4.054,4.054,0,0,0,6.8,1.96c.881-.784,1.7-1.639,2.535-2.471a2.4,2.4,0,0,1,1.582-.855,3.817,3.817,0,0,0,3.706-3.326,1.159,1.159,0,0,1,1.135-1.129A3.779,3.779,0,0,0,105.2,113c.145-.879.555-1.045,1.328-1.164a3.139,3.139,0,0,0,2.8-2.186c.41-2.031,1.738-3.469,2.716-5.132a12.279,12.279,0,0,0,1.7-6.379C113.639,92.85,111.345,88.728,106.6,86.2ZM76.551,110.063a2.084,2.084,0,0,1-1.956-1.212,1.867,1.867,0,0,1,.205-2.257c1.1-1.176,2.258-2.3,3.453-3.4a1.928,1.928,0,0,1,2.282-.2,2.029,2.029,0,0,1,1.207,1.924C81.73,106.321,77.963,110.051,76.551,110.063Zm6.036,3.789a2.06,2.06,0,0,1-3.139-.083,1.976,1.976,0,0,1,.036-2.958c1.014-1.069,2.077-2.1,3.175-3.1a2.162,2.162,0,0,1,3.562,1.675,1.943,1.943,0,0,1-.592,1.485C84.615,111.869,83.638,112.9,82.587,113.852Zm4.346,4.538a1.972,1.972,0,0,1-2.861-.059,1.951,1.951,0,0,1-.205-2.946c1.026-1.14,2.173-2.186,3.3-3.243a1.959,1.959,0,0,1,2.415-.261,2.057,2.057,0,0,1,1.171,1.948,1.887,1.887,0,0,1-.507,1.342C89.143,116.252,88.08,117.369,86.933,118.39Zm3.187,4.954a2.19,2.19,0,0,1-2-1.117,1.889,1.889,0,0,1,.205-2.388c1.087-1.176,2.258-2.281,3.441-3.362a1.892,1.892,0,0,1,2.27-.19A2,2,0,0,1,95.24,118.2C95.228,119.554,91.509,123.3,90.121,123.344Zm17.059-13.911c-1.1,1.045-2.282.772-3.32-.226-2.535-2.471-5.046-4.966-7.582-7.436-.543-.535-1.147-1.485-1.992-.689s.048,1.426.6,1.972c2.427,2.4,4.877,4.787,7.3,7.2,1.437,1.426,1.618,2.708.459,3.659-1.364,1.117-2.5.546-3.6-.558-2.451-2.471-4.95-4.882-7.437-7.318-.531-.511-1.1-1.271-1.859-.5-.749.748,0,1.354.507,1.865q3.767,3.742,7.582,7.436c1.074,1.045,1.485,2.138.4,3.4a1.215,1.215,0,0,1-.785.392c-.423-.012-.314-.44-.35-.725a3.856,3.856,0,0,0-3.417-3.635c-.736-.143-1.038-.38-1.147-1.129a3.79,3.79,0,0,0-3.32-3.3c-.7-.143-1.062-.3-1.171-1.081a3.743,3.743,0,0,0-3.3-3.326c-.857-.154-1.05-.511-1.244-1.259-.857-3.374-4.382-4.336-6.99-1.96a32.743,32.743,0,0,0-2.511,2.483c-.724.82-1.123.582-1.666-.2a12.341,12.341,0,0,1-2-7.377,11.046,11.046,0,0,1,6.085-9.206,11.555,11.555,0,0,1,11.638.143c1.328.713,1.823,1.342.362,2.459a22.049,22.049,0,0,0-2.4,2.328,3.991,3.991,0,0,0-.145,5.8,4.236,4.236,0,0,0,5.916,0c1.014-.915,1.992-1.877,2.922-2.875.6-.653,1.026-.689,1.666-.036C99.9,99.241,103.461,102.71,107,106.2,108.049,107.188,108.29,108.364,107.18,109.433Zm2.789-5.12c-.616,1.01-1.038,1.069-1.92.19-3.622-3.659-7.377-7.211-10.986-10.881-1.087-1.1-1.835-1.212-2.885-.012-1.1,1.259-2.354,2.388-3.574,3.54-1.05.986-2.246,1.212-3.308.131s-.845-2.233.205-3.255c1.642-1.6,3.187-3.326,4.95-4.787A11.285,11.285,0,0,1,104.91,87.48a10.976,10.976,0,0,1,6.93,9.658A12.619,12.619,0,0,1,109.969,104.313Z" transform="translate(-67.955 -83.941)" fill="#100abf" stroke="#100abf" stroke-width="1"/>
          </svg>          
          
        </div>
        <h4 slot="title">Devino partenerul nostru</h4>
        <div slot="content">
          <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer faucibus est non purus posuere feugiat. Sed nibh turpis, eleifend sit amet fermentum sed, cursus a ante. Morbi non eleifend massa. Maecenas nec facilisis lacus, in aliquam lacus.</p>
        </div>
      </DropDownListItem>
    </div>
  </section>
  <selection class="index__curent">
    <h2>Fii la curent cu ce facem!</h2>
    <div class="index__curent__icons">
      <svg id="Facebook" xmlns="http://www.w3.org/2000/svg" width="81.5" height="81.5" viewBox="0 0 81.5 81.5">
        <path id="Path_1217" data-name="Path 1217" d="M0,40.75A40.75,40.75,0,1,1,40.75,81.5,40.75,40.75,0,0,1,0,40.75Z" fill="#100abf"/>
        <path id="Path_1218" data-name="Path 1218" d="M32.156,56.335V34.165h6.12l.811-7.64H32.156l.011-3.824c0-1.993.189-3.06,3.051-3.06h3.826V12H32.923c-7.352,0-9.94,3.706-9.94,9.939v4.587H18.4v7.64h4.583V56.335Z" transform="translate(12.842 8.375)" fill="#fff"/>
      </svg>
      <svg id="Group_296" data-name="Group 296" xmlns="http://www.w3.org/2000/svg" width="81.5" height="81.5" viewBox="0 0 81.5 81.5">
        <path id="Path_1219" data-name="Path 1219" d="M55.081,23.88H30.791a7.04,7.04,0,0,0-7.032,7.032V55.2a7.04,7.04,0,0,0,7.032,7.032H55.081A7.041,7.041,0,0,0,62.114,55.2V30.912A7.04,7.04,0,0,0,55.081,23.88Zm-12.145,31.8A12.626,12.626,0,1,1,55.563,43.057,12.641,12.641,0,0,1,42.936,55.684ZM55.968,33.042a2.987,2.987,0,1,1,2.987-2.987A2.991,2.991,0,0,1,55.968,33.042Z" transform="translate(-2.186 -2.197)" fill="#100abf"/>
        <path id="Path_1220" data-name="Path 1220" d="M44.14,36.971a7.291,7.291,0,1,0,7.29,7.289A7.3,7.3,0,0,0,44.14,36.971Z" transform="translate(-3.39 -3.401)" fill="#100abf"/>
        <path id="Path_1221" data-name="Path 1221" d="M40.75,0A40.75,40.75,0,1,0,81.5,40.75,40.751,40.751,0,0,0,40.75,0ZM65.262,53.005A12.38,12.38,0,0,1,52.9,65.372H28.6A12.38,12.38,0,0,1,16.238,53.005V28.715A12.381,12.381,0,0,1,28.6,16.348H52.9A12.38,12.38,0,0,1,65.262,28.715Z" fill="#100abf"/>
      </svg>
      <svg id="YouTube" xmlns="http://www.w3.org/2000/svg" width="81.5" height="81.5" viewBox="0 0 81.5 81.5">
        <path id="Path_1222" data-name="Path 1222" d="M0,40.75A40.75,40.75,0,1,1,40.75,81.5,40.75,40.75,0,0,1,0,40.75Z" fill="#100abf"/>
        <path id="Path_1223" data-name="Path 1223" d="M53.758,20.078a5.518,5.518,0,0,0-3.843-3.946C46.526,15.2,32.933,15.2,32.933,15.2s-13.593,0-16.982.933a5.518,5.518,0,0,0-3.843,3.946C11.2,23.559,11.2,30.821,11.2,30.821s0,7.262.908,10.742a5.519,5.519,0,0,0,3.843,3.946c3.39.932,16.982.932,16.982.932s13.593,0,16.982-.932a5.519,5.519,0,0,0,3.843-3.946c.908-3.481.908-10.742.908-10.742S54.667,23.559,53.758,20.078Z" transform="translate(7.817 10.608)" fill="#fff"/>
        <path id="Path_1224" data-name="Path 1224" d="M21.6,34.383V20.8l10.867,6.792Z" transform="translate(15.075 14.517)" fill="#100abf"/>
      </svg>
      <svg id="Pinterest" xmlns="http://www.w3.org/2000/svg" width="81.5" height="81.5" viewBox="0 0 81.5 81.5">
        <circle id="Ellipse_328" data-name="Ellipse 328" cx="40.75" cy="40.75" r="40.75" transform="translate(0 0)" fill="#100abf"/>
        <g id="Group_295" data-name="Group 295" transform="translate(21.065 17.863)">
          <path id="Path_1225" data-name="Path 1225" d="M51.974,61.3c-3.081-.238-4.375-1.766-6.791-3.233-1.328,6.967-2.951,13.646-7.757,17.134C35.94,64.67,39.6,56.76,41.3,48.365c-2.9-4.881.349-14.707,6.465-12.286,7.528,2.977-6.517,18.151,2.911,20.046,9.844,1.979,13.862-17.08,7.757-23.279-8.82-8.95-25.673-.2-23.6,12.609.5,3.132,3.742,4.083,1.293,8.405-5.645-1.25-7.329-5.7-7.112-11.638.349-9.716,8.73-16.518,17.136-17.46,10.631-1.19,20.609,3.9,21.986,13.9,1.549,11.286-4.8,23.509-16.166,22.631Z" transform="translate(-29 -24.593)" fill="#f1f2f2"/>
        </g>
      </svg>                        
    </div>
  </selection>
</div>

<style>
  .wrapper {
  }
  .index__main {
    height: 100vh;
    padding-top: 72px;
    padding-left: 2rem;
    padding-right: 2rem;
    display: flex;
    flex-direction: column;
    justify-content: center;
    background-image: url("/images/index/row-1-col-1.png");
    background-position: top;
    background-repeat: no-repeat;
    background-size: cover;
  }
  .index__main > p {
    max-width: 480px;
  }
  @media only screen and (max-width: 720px) {
    h2 {
      font-size: 1.75rem;
    }
    .index__button__wrapper {
      padding-top: 1rem;
    }
  }
  @media only screen and (min-width: 720px) {
    .index__main {
      padding: 3rem 10rem;
    }
  }
  /* SECOND SECTION */
  .index__7ani {
    height: 100vh;
    display: flex;
    flex-direction: column;
    padding: 3rem;
    align-items: flex-end;
    background-image: url("/images/index/row-2-col-1.png");
    background-position: top;
    background-repeat: no-repeat;
    background-size: cover;
  }

  .index__7ani > p {
    max-width: 480px;
  }
  .index__7ani__list {
    padding: 0 0;
  }
  .index__7ani__listitem{
    margin: 1rem 0;
    display: flex;
  }
  .index__7ani__listitem__textarea{
    display: flex;
    flex-direction: column;
    padding-left: 1rem;
  }
  .index__7ani__listitem__textarea > h4 {
    font-size: 1.5rem;
    padding: 0;
    margin: 0;
  }
  .index__7ani__listitem__textarea > p {
    margin: 0;
    padding: 0;
  }
  .index__7ani__wrapper {
    max-width: 600px;
    width: 100%;
    height: 100%;
    display: flex;
    flex-direction: column;
    justify-content: center;
  }
  .index__7ani__wrapper > p {
    max-width: 480px;
  }

  /* Third section */
  .index__activitati {
    height: 100vh;
    display: flex;
    flex-direction: column;
    align-items: center;
    padding: 3rem;
    padding-top: 5rem;
    background-image: url("/images/index/row-3-col-1.png");
    background-position: top;
    background-repeat: no-repeat;
    background-size: cover;
  }
  .index__activitati__cardarea {
    margin-top: 5rem;
    padding: 2rem;
    display: flex;
    width: 100%;
    flex-direction: row;
    justify-content: space-evenly;
    overflow-x: auto;
  }
  @media only screen and (max-width: 720px) {
    .index__activitati__cardarea {
      justify-content: flex-start;
    }
  } 

  /* FOrth section */
  .index__implicare {
    height: 100vh;
    width: 100%;
    display: flex;
    flex-direction: column;
    align-items: center;
    padding-top: 5rem;
    background-image: url("/images/index/row-4-col-1.png");
    background-position: top;
    background-repeat: no-repeat;
    background-size: cover;
  }
  .index__implicare__list {
    padding-top: 5rem;
    width: 70%;
  }
  .index__curent {
    height: 40vh;
    width: 100%;
    display: flex;
    flex-direction: column;
    align-items: center;
    padding-top: 5rem;
  }
  .index__curent__icons {
    width: 100%;
    display:flex;
    flex-direction: row;
    justify-content: center;
  }
  .index__curent__icons > svg {
    margin: 1rem;
  }
</style>
